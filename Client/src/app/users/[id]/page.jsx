...
async function getUser(id) {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_APIURL}/users/${id}`
    );
    const data = response.data;
    return data;
  } catch (error) {
...
export default async function UserPage({ params }) {
  ...
  const { selectedType } = await getUser(params.id);
  ...
  return (
    <div>
      {selectedType === "owner" ? (
        <div>
          <NavBarHome />
          <div className=" h-screen border border-indigo-300 m-4 p-4">
            {user && (
              <div className="p-4">
                <h1 className="text-2xl mb-4 font-bold text-indigo-500">
                  User Profile
                </h1>
                <UserDetail
                  noButton
                ...
                {user.isWalker === true ? (
                  <div className="mt-4">
                    ...
                    {user.dog_count >= 1 ? (
                      ...
                      user.dogs.map((dog) => (
                        <div
                          key={dog.id}
                          className="border border-indigo-300 rounded p-4 my-2"
                        >
                      ...
                      <p className="mb-2">
                        No dogs have been added to your profile.
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="mt-4">
                    ...
                    <p className="mb-2">
                      Dog Capacity:{" "}
                      {user.dog_capacity || "No information added"}
                    </p>
                    <p className="mb-2">
                      Is Available: {user.is_available ? "Yes" : "No"}
                    </p>
                  </div>
                )}
              </div>
